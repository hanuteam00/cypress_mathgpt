...
Cypress.Commands.add('loginByPredefinedAccessToken_expMathGPTUrl', (access_token_new_educator) => {
  // cy.session([email, password], () => {
  //step 1 (mandatory): send request to get token
  //create session - new M3 release
  cy.request({
    url: "https://mathgpt-api-exp.tutoruniverse.net/sign-in",
    method: 'POST',
    headers: {
      contentType: "application/json",
    },
    body: { "email": "manh+edu1@gotitapp.co"}

  }).then(res => {
    window.localStorage.setItem('gotit.mathgpt.authenticated_access_token', access_token_new_educator);
